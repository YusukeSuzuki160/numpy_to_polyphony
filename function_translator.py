...
class FunctionTranslator(ast.NodeTransformer):
                ...
                if node.func.attr == 'array':
                    if isinstance(node.args[0], ast.Name):
                        if node.args[0].id in self.array_list.keys():
                            ...
                            return ast.copy_location(ast.Call(func=ast.Name(id=function_name, ctx=ast.Load()), args=node.args, keywords=[]), node)
    ...
    def visit_BinOp(self, node: ast.BinOp) -> Any:
        ...
        if function_name != '':
            ...
            return ast.copy_location(ast.Call(func=ast.Name(id=function_name, ctx=ast.Load()), args=args, keywords=[]), node)
    ...
    def flaot_to_int(self, node: ast.BinOp) -> Any:
        ...
        if function_name != '':
            self.func_dict[function_name] = {'dtype': dtype}
            return ast.copy_location(ast.Call(func=ast.Name(id=function_name, ctx=ast.Load()), args=[node.left, node.right], keywords=[]), node)
