...
class NpLibGenerator:  # run at the super directory of polyphony-numpy
    ...
    def generate_array(self, func):
        ...
        func_str = (
            "def " + function_name + "(" + arg_list + ") -> " + return_type + ":\n"
        )
        func_str += "\t" + return_stm
                ...
                for k in range(0, shape[1], 2):
                    if j == 0:
                        func_str += (
                            ...
                            + ", t0_imag_"
                            + str(i * shape[1] + k)
                            + " = complex128_add(a_real_"
                            + str(i * shape[1] + k // 2)
                            + ", a_imag_"
                            + str(i * shape[1] + k // 2)
                            + ", a_real_"
                            + str(i * shape[1] + k // 2 + but_interval)
                            + ", a_imag_"
                            + str(i * shape[1] + k // 2 + but_interval)
                            + ")\n"
                        )
                        ...
                        func_str += (
                            ...
                            + str(i * shape[1] + k // 2 + but_interval)
                            + ", "
                            + str(w_real[bit_rev_w[((k // 2) % (2**j))]])
                            + ", "
                            + str(w_imag[bit_rev_w[((k // 2) % (2**j))]])
                            + ")\n"
                        )
                        func_str += (
                            "\tt"
    ...
    def generate_matmult(self, func):
        ...
        func_str = (
            "def " + func_name + "(" + arg_list + ") -> " + return_type[:-2] + "]:\n"
        )
            ...
            for j in range(return_shape[1]):
                for k in range(left_shape[1]):
                    func_str += (
                        ...
                        + str(count)
                        + " = a_"
                        + str(i * left_shape[1] + k)
                        + " * b_"
                        + str(k * right_shape[1] + j)
                        + "\n"
                    )
                    count += 1
        count = 0
    ...
    def fixed_mul(self, type):
        if type == "fixed64":
            self.code += (
                "def fixed64_mult(a: int64, b: int64) -> int64:\n\
    temp = a * b\n\
    ...
    def complex_mul(self, type):
        if type == "complex64":
            self.code += (
                "def complex64_mult(a_real: int32, a_imag: int32, b_real: int32, b_imag: int32) -> Tuple[int32, int32]:\n\
